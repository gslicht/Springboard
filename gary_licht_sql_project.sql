...
SELECT COUNT( membercost ) AS count_member_cost_zero
FROM `Facilities`
WHERE membercost =0
...
IN ( 0, 1 )
GROUP BY CONCAT(full_name,' ', court_name)
ORDER BY 1, 2
...
FROM (
SELECT Bookings.memid AS memid, Facilities.name AS fac_name, Bookings.starttime AS
START ,
CASE WHEN Bookings.memid =0
THEN Bookings.slots * Facilities.guestcost
ELSE Bookings.slots * Facilities.membercost
...
SELECT Facilities.name, z.guest_slot * Facilities.guestcost + z.member_slot * Facilities.membercost AS total_revenue

FROM (
SELECT Bookings.facid AS facid, SUM(
CASE WHEN Bookings.memid =0
THEN slots
ELSE 0
END ) AS guest_slot, SUM(
CASE WHEN Bookings.memid !=0
THEN slots
ELSE 0
END ) AS member_slot
